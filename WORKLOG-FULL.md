...
### #132 기록 탭 진입 시 '오늘' 자동 앵커 (A안)
- **요청**: 기록 탭 진입 시 오늘이 기준 화면이었는데, 선반영 도입 후 월의 가장 늦은 날짜(미래 예정)가 최상단에 보이는 문제 점검 → A안(오늘 자동 앵커) 승인
- **진단**: 코드 회귀 아님 — 자동 스크롤은 원래 `?scrollTo=` 딥링크 전용이었고, 이전에 오늘이 보였던 건 미래 거래가 없어 내림차순 최상단=오늘이었기 때문. 선반영이 실제 작동하며 드러난 UX 공백
- **변경** (`src/pages/HistoryPage.tsx`):
  1. `scrollToGroupBySelector`에 `clearParams` 옵션·발견 여부(boolean) 반환 추가 — 딥링크 모드만 URL 파라미터 정리
  2. 무파라미터 진입 시 1회 자동 앵커 효과 신설: 현재 월 + 검색/카테고리/인사이트 파라미터 없음 조건에서 '오늘' 그룹으로 스크롤, 오늘 기록 없으면 '가장 가까운 예정' 그룹 폴백. 가드는 실행 시점에 세워 StrictMode 이중 마운트에서도 동작
  3. 스크롤 계산을 rAF 의존 없이 동기화 — 리셋+다음 프레임 대기 제거(현재 scrollTop 반영 수식으로 어느 위치에서든 정확, 백그라운드 탭에서도 결정적)
- **검증** (실브라우저, dev 서버 + 시드 229건 + 미래 거래 2건 주입):
  - 무파라미터 진입 → scrollTop 292, '오늘' 헤더가 고정 헤더 바로 아래(116px) 배치 ✓
...
### #134 Phase 2 방향 확정 + 실행 계획 문서화
- **요청**: 상용화 4·5단계(iOS 채널·결제) 논의 → Mac mini M4 보유 확인 → 개발 보류하고 방향 확정에 집중
- **논의·결정**:
  - P-1 빌드 환경: Mac mini M4 로컬 빌드 (클라우드 빌드 불필요)
  - P-2(D3) 출시 순서: 한국 선행 — 한국어 그대로 TestFlight, i18n은 한국 리텐션 검증 후로 연기
  - P-3 위젯 v1: 홈 위젯 2종 (Small 잔액 / Medium 잔액+하루 가용액), 잠금화면은 v1.1
  - P-4 PWA: 무료 티어로 유지 (iOS 앱이 유료 기능 채널)
...
### #136 분석 카테고리 리스트 — 예산/금액 보기 기준 전환
- **요청**: ① 카테고리 막대를 전체 지출 구성비가 아닌 월 예산 대비로 ② 구성비·예산비 텍스트 혼재로 빨간 '예산의 100%(초과)'가 인지적 오해를 만듦 ③ 정렬을 금액순 → 예산순으로 (고정지출이 늘 1위라 변동 소비 패턴이 안 보임) ④ 예산순=예산 대비, 금액순=총지출 대비로 기준을 함께 전환
- **결정 (사용자 선택)**:
  - 예산 미설정 카테고리: 회색 구성비 막대 + '예산 미설정' 표기 (수입·연간은 기존 유지)
  - 우상단 % 텍스트: 예산순일 때 '예산의 X%'로 교체 (구성비는 도넛 차트가 담당)
  - '금액순 | 예산순' 토글 추가, 기본값 예산순
- **변경** (`src/pages/StatsPage.tsx`):
  - `categoryViewMode` state + `hasCategoryBudget()` 헬퍼(모듈 스코프) 추가
  - `isBudgetViewAvailable` = 월간 + 지출 + **예산 설정된 카테고리 1개 이상** — 예산 미설정 사용자는 토글 숨김·기존 동작 유지
  - `categoryListItems`: 예산순일 때 예산 있는 항목 우선 → 사용률 내림차순, 예산 없는 항목은 하단 금액순. **도넛·인사이트·필터칩은 금액순 원본(`activeCategoryBreakdown`) 그대로** 사용
  - 막대: 예산순 = `min(budgetPercent,100)` 카테고리색 / 초과 시 red-500 / 예산 없으면 회색 구성비, 금액순 = 기존 구성비
  - 하단 예산 문구 제거 — 한 행에 기준이 하나만 남음. 100% 정확히 소진에 '(초과)'가 붙던 표기도 해소(초과는 `> 100`)
...
- **배포**: 커밋 0f9671d → feat·main 푸시(ff, 원격 main이 pinpig 히스토리인지 사전 확인 — #135 재발 방지) → `vercel --prod` (dpl_ErfAQy3fw3ZxnUmtGGyuAm9d3HzM, READY, pinpig.vercel.app 별칭)
- **배포 검증**: SW 프리캐시 매니페스트로 33개 청크 수집 후 문자열 확인 — `기록될 예정이에요`(index), `"예정 ",dx(r)," 포함"`(HeroSection), 예정 배지(HistoryPage·EditTransactionPage·TransactionDetailPage). **프로덕션 실동작 확인** — 확정 30만+예정 50만 입력 시 홈 1,200,000원 + "예정 500,000원 포함", 분석 탭 300,000원(15%)만 집계
- **⚠ 잔여**: 검증용 테스트 데이터가 이 PC Chrome의 pinpig.vercel.app 로컬 DB에 남음(온보딩 완료 + 예산 200만 + 거래 2건). 앱의 모든 삭제 경로가 `window.confirm`을 사용해 브라우저 자동화로 정리 불가 — **설정 → 데이터베이스 초기화**로 수동 정리 필요. 다른 기기·프로필에는 영향 없음(로컬 전용 IndexedDB)
- **결과**: 완료 · 배포됨

### #138 제품 문서 시각화 — PRD · 유저플로우 · IA
- **요청**: `참고/` 내 HTML 2종(BRIS 대시보드 유저플로우, dash_erp 구조도)의 문서 시각화 방식을 참고하여 기존 PRD·유저플로우·IA를 시각화 문서로 재구성
- **역설계** (참고 파일 2종에서 규격 추출):
  - 캔버스: `#f7f7f9` + 24px 점격자, 드래그 팬 · 휠 줌, HUD(화면 맞춤 · 100%)
  - 그리드: 행 44 · 노드 높이 32 · 컬럼 간격 64 · 상단 여백 48 / 레인 간격 = 하단+11(구분선), +27(다음 레인)
  - 노드 4종: `start #ececf0` · `hub #7461e6` · `leaf #efeafc/#ddd3fb` · `action #1e1e22 pill(rx16)`
  - 커넥터: `#c5c5d2` 1.5px 엘보, midx=(출발 오른쪽+도착 왼쪽)/2, 반경 `min(7,|dy|/2)`
  - 배치: 트리 레이아웃 — 잎은 연속 슬롯, 부모는 자식 span의 중앙 (참고 파일의 좌표에서 확인)
- **산출** (`docs/product/`):
  - `PinPig_PRD_구조도.html` — 9레인 91노드 (정의·프레임 / 타겟 / 문제 / 해결 / 차별화 / In Scope 12영역 / Out of Scope / 지표·리스크 / 플랫폼·스택)
  - `PinPig_유저플로우.html` — 8레인 107노드 (진입 게이트 / 1초 확인 / 3터치 기록 / 내역 확인 / 월말 리뷰 / 예산 3경로 / 반복거래 라이프사이클 / 알림 트리거)
  - `PinPig_IA_화면구조.html` — 7레인 83노드, **27개 라우트 전량** + 설정 5영역 계층 + 로컬 저장 계층
  - 원본 `.md` 4종은 유지하고 상단에 시각화 문서 링크 추가
- **생성기** (`scripts/docviz/`): `render.py`(시각 체계·레이아웃·출력) + `content.py`(내용만) + `README.md`. **HTML 직접 수정 금지 — content.py 고쳐 재생성**
- **내용 기준**: v0.2.5 + #136(금액순/예산순 토글) + #137(예정 거래 정합화) 반영 — 예정 배지·"예정 N원 포함"·집계 기준 분리·알림 기준 비대칭까지 문서에 명시
...
### #140 예정 거래 도래 후 시한부 '확인' 배지 + 잔재 정리
- **요청**: #139에서 남긴 판단 필요 4건 정리 → 각 항목 상세 설명 후 결정
- **결정 (사용자 선택)**:
  - ① 예정 도래 — **시한부 배지** (사용자 제안). "조용히 경고하되 무응답이면 확정으로 판단"
  - ② 검색 — 현행 유지 / ③ 카테고리 정리 — 현행 유지 / ④ `HeroCarousel` — 삭제
- **② 판단 근거 (실측)**: 벤치마크 결과 1,000건 22ms · 5,500건(5년) 105ms · 15,000건(14년) 320ms — 정확히 선형. 입력에 이미 300ms 디바운스가 있어 5년치까지 여유. 측정 후 벤치마크 파일 삭제
- **① 구현 — 스키마 변경 0**: 저장 필드를 늘리지 않고 기존 값만으로 판정 (`needsSettlementCheck`)
  - `createdAt < date` → 발생 전에 미리 넣은 거래 (당일·소급 입력은 제외됨)
  - `updatedAt < date` → 도래 후 손대지 않음. **수정하면 확인한 것으로 간주해 배지 소멸**
  - `tags`에 '반복' 있으면 제외 — 반복 자동 생성분은 금액 고정 + 별도 알림 존재
  - 시한 7일 (`SETTLEMENT_CHECK_WINDOW_DAYS`) — 지나면 조용히 사라지고 그대로 확정
  - 표시: 기록 탭 행 앰버 "확인" 배지(파란 "예정"과 색으로 구분), 홈 하단 "확인" 카드 → `/history?scrollTo=check`
  - `data-scroll-check` 별도 속성 사용 — 확인 대상 그룹이 today/yesterday와 겹칠 수 있어 `data-scroll-target`과 분리
- **④**: `HeroCarousel.tsx`(357줄) 삭제 + barrel 정리 — #139의 `BudgetInsightCard`와 같은 종류의 잔재
- **문서**: `docs/UPCOMING_TRANSACTIONS.md` §2.1.1 신규(판정 조건표), 시각화 문서 재생성
...
### #141-배포 빌드 재실행 + 프로덕션 배포
- **요청**: 빌드 재실행 및 배포
- **빌드**: 이전 세션의 OOM은 시스템 커밋 메모리 고갈이었고, 재실행 시 정상 통과 (11.8s, precache 45 entries)
- **푸시**: 커밋 606a229 → feat·main 푸시(ff, origin/main이 HEAD 조상인지 사전 확인 — #135 재발 방지). main에는 #138·#139·#140·#141 네 건이 함께 반영됨
- **계정 문제**: CLI·MCP가 daun92 계정으로 잡혀 프로젝트 403. 디바이스 로그인 3회가 모두 브라우저 기본 세션(daun92)으로 승인됨 → 사용자가 `! npx vercel login`으로 daunny 승인 후 해결. **pinpig 배포는 반드시 daunny 계정** (#133 메모 재확인)
...
- **결과**: 완료 · 커밋 3804c10 → feat·main 병합(ff) · **배포 완료**
- **배포**: `vercel --prod` (daunny) → dpl_BwW8vkcfMrw3Cn2pr7b3ASGkwd1i, production READY (2026-09-07 11:39 KST). pinpig.vercel.app 번들 `index-B_JQ3DL-.js`에 빌드 커밋 dc324d6·백업 카드·오류 경계 문자열, SettingsPage 청크에 "익명 사용 통계·지금 백업" 확인
